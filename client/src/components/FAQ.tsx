...
export default function FAQ() {
  ...
  return (
    <section id="faq" className="py-24" style={{ backgroundColor: "#E8F5F7" }}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="inline-block px-4 py-2 rounded-full mb-6" style={{ backgroundColor: "rgba(30, 123, 140, 0.15)" }}>
            <span className="text-sm font-medium" style={{ color: "#0D4A5A" }}>Depoimentos</span>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {testimonials.map((item, index) => (
            ...
            >
              <Quote className="w-8 h-8 mb-4 opacity-20" style={{ color: "#1E7B8C" }} />
              <p className="text-base leading-relaxed mb-6" style={{ color: "#3C3C3C" }}>
                "{item.text}"
              </p>
              <div className="flex items-center gap-1 mb-3">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star key={star} className="w-4 h-4 fill-current" style={{ color: "#1E7B8C" }} />
                ))}
              </div>
              <div>
                <p className="font-semibold text-sm" style={{ color: "#212529" }}>{item.name}</p>
                <p className="text-xs" style={{ color: "#1E7B8C" }}>{item.detail}</p>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        >
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-8 text-lg" style={{ color: "#E8F5F7" }}>
            <span>Atendimento particular</span>
            <span>|</span>
            <span>Jardim das Perdizes — São Paulo/SP</span>
          </div>
          <p className="text-base mb-8" style={{ color: "rgba(232, 245, 247, 0.8)" }}>
